...
const Header = () => {
  ...
  return (
    <header className="bg-black sticky top-0 z-50">
      ...
      <div className="container mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-6">
            {menuItems.map((item, index) => (
              <div key={index} className="relative group">
                <button className="text-gray-200 hover:text-white font-medium py-2 flex items-center gap-1 text-[14px]">
                  {item.name}
          ...
          <div className="flex items-center absolute left-1/2 -translate-x-1/2">
            <h1 className="text-2xl md:text-3xl text-gray-200 tracking-wider" style={{ fontFamily: "'Playfair Display', serif", fontWeight: 400 }}>
              Xhara
            </h1>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <button 
              className="p-2 hover:bg-gray-900 rounded-full relative transition-colors"
              onClick={() => setIsCartOpen(!isCartOpen)}
            >
              <FiShoppingCart className="w-5 h-5 text-gray-200" />
          ...
          <nav className="container mx-auto px-4 py-4">
            {menuItems.map((item, index) => (
              <div key={index} className="py-2">
                <a href="#" className="text-gray-200 font-medium py-2 flex items-center gap-2">
                  {item.name}
                ...
                {item.submenu && (
                  <ul className="pl-4 mt-2 space-y-1">
                    {item.submenu.map((subItem, subIndex) => (
                      <li key={subIndex}>
                        <a href="#" className="text-sm text-gray-400 block py-1 hover:text-gray-200">
                          {subItem}
                        </a>
                      </li>
                    ))}
      ...
      {/* Cart Dropdown */}
      {isCartOpen && (
        <div className="absolute top-full right-4 mt-2 w-80 bg-black border border-gray-700 shadow-xl rounded-lg p-4 z-50">
          <div className="text-center py-8">
            <p className="text-gray-200 mb-4">Item added to your cart</p>
            <div className="flex gap-2 justify-center">
              <button className="px-4 py-2 bg-white text-black rounded hover:bg-gray-200 transition-colors">
                View cart
              </button>
              <button className="px-4 py-2 border border-gray-600 text-gray-200 rounded hover:bg-gray-900 transition-colors">
                Check out
              </button>
            </div>
            <button 
